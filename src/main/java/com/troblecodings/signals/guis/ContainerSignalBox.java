...
public class ContainerSignalBox extends ContainerBase implements UIClientSync, IChunkLoadable {
	...
	public void sendAllDataToRemote() {
		...
		positions.entrySet().stream().filter(entry -> entry.getValue().equals(LinkType.SIGNALBOX)).forEach(entry -> {
			...
			if (grid.get() == null) {
				loadChunkAndGetTile(SignalBoxTileEntity.class, (ServerLevel) info.world, entry.getKey(),
	...
	public void deserializeClient(final ReadBuffer buffer) {
		...
		case ADDED_TO_SAVER: {
			...
			nextPathways.put(Maps.immutableEntry(start, end), type);
			infoUpdates.accept(
					I18Wrapper.format("error." + result.getName()) + " - " + I18Wrapper.format("info.pathwaysaver"));
			break;
		}
		case OUTPUT_UPDATE: {
		...
		case SET_SIGNAL_STATE: {
			...
			for (int i = 0; i < greenSignalSize; i++) {
				...
				if (entry != null) {
					modeIdentifier.state = SignalState.combine(entry.enumValue.getSubsidiaryShowType());
				}
				grid.getNodeChecked(modeIdentifier.getPoint()).ifPresent(node -> {
					...
					this.updatePoint.accept(modeIdentifier.getPoint(), modeIdentifier.getModeSet());
				});
			}
			break;
		}
	...
	public void deserializeServer(final ReadBuffer buffer) {
		...
		case REMOVE_ENTRY: {
			...
			grid.getNode(point).getOption(modeSet).ifPresent(entry -> entry.removeEntry(entryType));
			break;
		}
		case SEND_POS_ENTRY: {
		...
		case REMOVE_POS: {
			final BlockPos pos = buffer.getBlockPos();
			SignalBoxHandler.unlinkPosFromSignalBox(new StateInfo(tile.getLevel(), tile.getBlockPos()), pos);
			break;
		}
		case RESET_PW: {
			final Point point = Point.of(buffer);
		...
		case REQUEST_PW: {
			...
			if (!request.isPass()) {
				...
				if (request.canBeAddedToSaver() && type.equals(PathType.NORMAL) && !endNode.containsOutConnection()
